...
// The component receives the item to edit and a function to close the form
export default function EditMenu({ itemToEdit, onFinished }) {
  ...
  // useEffect pre-fills the form when the component receives a new item to edit.
  useEffect(() => {
    if (itemToEdit) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const updatedItem = {
      ...itemToEdit, // Keep the original ID
      name,
      price,
      category,
      image: imagePreview,
    ...
    onFinished(); // Close the form after submitting
  };

  if (!itemToEdit) return null; // Don't render if no item is selected
