  ...
  return (
    <aside className="h-full w-full bg-gray-900/80 backdrop-blur-sm text-white p-6 rounded-lg shadow-lg border border-gray-700 flex flex-col">
      ...
      <form onSubmit={handleSubmit} className="flex-1 flex flex-col space-y-4">
          ...
          {imagePreview ? (
            <>
              ...
              <button
                type="button"
                onClick={clearImage}
                className="absolute top-2 right-2 bg-red-600/80 hover:bg-red-500 rounded-full p-1"
              >
                <X size={16} />
              </button>
            </>
        ...
        <button
          type="submit"
          className="mt-auto w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-4 rounded-lg transition"
        >
          Add Item to Menu
        </button>
      </form>
